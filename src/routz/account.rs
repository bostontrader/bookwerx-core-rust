use crate::db::{Account, AccountDenormalized, AccountJoined, AccountShort, APIResponse, GetAccountResponse, MyRocketSQLConn};
use rocket::http::RawStr;
use rocket_contrib::json::Json;

#[rocket::delete("/account/<id>?<apikey>")]
pub fn delete_account(id: &RawStr, apikey: &RawStr, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
    ...
    match conn.prep_exec("DELETE from accounts where id = :id and apikey = :apikey",params) {
...
pub fn get_account(id: &RawStr, apikey: &RawStr, mut conn: MyRocketSQLConn) -> Json<GetAccountResponse> {
    ...
    match vec.len() {
        ...
        _ => Json(GetAccountResponse::Error(String::from("ID01T Max fubar error. More than one record found. This does not compute.")))
    }
}
...
pub fn post_account(account: rocket::request::Form<AccountShort>, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
...
pub fn put_account(account: rocket::request::Form<Account>, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
