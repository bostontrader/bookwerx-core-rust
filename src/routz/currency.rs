...
pub fn delete_currency(id: &RawStr, apikey: &RawStr, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
    ...
    match conn.prep_exec("DELETE from currencies where id = :id and apikey = :apikey",params) {
...
pub fn get_currency(id: &RawStr, apikey: &RawStr, mut conn: MyRocketSQLConn) -> Json<GetCurrencyResponse> {
    ...
    match vec.len() {
        ...
        _ => Json(GetCurrencyResponse::Error(String::from("ID01T Max fubar error. More than one record found. This does not compute.")))
    }
}
...
pub fn get_currencies(apikey: &RawStr, mut conn: MyRocketSQLConn) -> Json<GetCurrencyResponse> {
    ...
    let vec: Vec<Currency> =
        conn.prep_exec("SELECT id, apikey, rarity, symbol, title from currencies where apikey = :apikey", params)
            .map(|result| {
                result.map(|x| x.unwrap()).map(|row| {
                    let (id, apikey, rarity, symbol, title) = rocket_contrib::databases::mysql::from_row(row);
                    Currency {id,apikey,rarity,symbol,title}
...
pub fn post_currency(currency: rocket::request::Form<CurrencyShort>, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
...
pub fn put_currency(currency: rocket::request::Form<Currency>, mut conn: MyRocketSQLConn) -> Json<APIResponse> {
