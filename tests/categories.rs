...
pub fn categories(client: &Client, apikey: &String) -> Vec<D::Category> {
    ...
    let mut response = client
        .get(format!("/categories?apikey={}", &apikey))
        .dispatch();
    ...
    response = client
        .put("/categories")
        .body(format!(
            "apikey={}&id={}&symbol=A&title=Assets",
            apikey, lid
        ))
        .header(ContentType::Form)
        .dispatch();
    ...
    response = client
        .get(format!("/category/{}/?apikey={}", lid, apikey))
        .dispatch();
    assert_eq!(response.status(), Status::Ok);
    match serde_json::from_str(&(response.body_string().unwrap())[..]).unwrap() {
    ...
    response = client
        .post("/categories")
        .body(format!(
            "apikey={}&symbol=C&title=Specific customer",
            apikey
        ))
        .header(ContentType::Form)
        .dispatch();
    ...
    response = client
        .get(format!("/category/bysym/{}/?apikey={}", "A", apikey))
        .dispatch();
    assert_eq!(response.status(), Status::Ok);
    match serde_json::from_str(&(response.body_string().unwrap())[..]).unwrap() {
        D::GetCategoryResponse::One(_) => assert!(true),
    ...
    response = client
        .get(format!("/category/bysym/{}/?apikey={}", "666", apikey))
        .dispatch();
    assert_eq!(response.status(), Status::Ok);
    match serde_json::from_str(&(response.body_string().unwrap())[..]).unwrap() {
