...
fn conn_no_cli_with_env() -> Result<(), Box<std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Accessing the db via connection string mysql://root:supersecretpassword@172.17.0.2:3306, as set from the environment."));
...
fn conn_with_cli_no_env() -> Result<(), Box<std::error::Error>> {
    ...
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Accessing the db via connection string mysql://root:supersecretpassword@172.17.0.2:3306, as set from the command line."));
    Ok(())
}
